...
const Signup = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault()
    try {
      await axios.post('https://med-care-srirams-projects-4f261a65.vercel.app/signup', form)
      navigate('/login')
    } catch (err) {
  ...
  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm bg-white rounded-lg h-480px inset-shadow-sm shadow-gray-500 p-[25px]">
    <form action="#" method="POST" onSubmit={handleSubmit} class="space-y-6 rounded-sm h-96 p-[25px] h-432px">
      <div>
        <label for="name" class="block text-sm/6 font-medium text-black-100">Name</label>
        <div class="mt-2">
          <input id="name" type="name" onChange={handleChange} value={form.name} name="name" required autocomplete="name" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-black border border-black outline-1 -outline-offset-1 outline-black/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
        </div>
      </div>
      <div>
        <label for="email" class="block text-sm/6 font-medium text-black-100">Email address</label>
        <div class="mt-2">
          <input id="email" type="email" onChange={handleChange} value={form.email} name="email" required autocomplete="email" class="block w-full rounded-md bg-white/5 px-3 py-1.5 text-base text-black border border-black outline-1 -outline-offset-1 outline-black/10 placeholder:text-gray-500 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-500 sm:text-sm/6" />
        </div>
      </div>
